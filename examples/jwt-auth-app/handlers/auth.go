...
type Authenticator interface {
	Login(ctx context.Context,
		...
		tokenIssuer string) (*auth.AccessAndRefreshToken, error)
	Refresh(ctx context.Context,
		token, tokenIssuer string) (*auth.AccessAndRefreshToken, error)
...
func (c *authController) Login(w http.ResponseWriter, r *http.Request) {
	...
	tokens, err := c.authenticator.Login(r.Context(),
		data.Email, data.Password, TokenIssuer)
	if err != nil {
...
func (c *authController) Refresh(w http.ResponseWriter, r *http.Request) {
	...
	tInd := slices.IndexFunc(cookies, func(c *http.Cookie) bool {
		return c.Name == RefreshTokenName
	})
	if tInd == -1 {
		log.Printf("[ERROR] Missing refresh token\n")
	...
	refreshToken := cookies[tInd].Value
	if refreshToken == "" {
		log.Printf("[ERROR] Empty refresh token\n")
	...
	tokens, err := c.authenticator.Refresh(r.Context(), refreshToken, TokenIssuer)
	if err != nil {
...
func (c *authController) Logout(w http.ResponseWriter, r *http.Request) {
	...
	err = c.authenticator.Logout(r.Context(), refreshToken)
	if err != nil {
